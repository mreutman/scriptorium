...
class Psalms:
  ...
  # Helper function for reading in CSV files into Dict
  def __read_csv(self, file_path, la, en):
    f = open(file_path, 'r')
    line = f.readline()
...
if __name__=='__main__':  
  ...
  for n in range(1, 151):
    la, _ = ps.GetChapter(n)
    insc, _ = ps.GetChapterInscription(n)

    if (current_kathisma != kathisma[n]):
      current_kathisma = kathisma[n]
      current_stasis = stasis[n]
      cmd = cmd_psalm_division
      cmd = cmd.replace("$1", str(current_kathisma))
      cmd = cmd.replace("$2", str(current_stasis))
      f.write(cmd)
    elif (118 != n):
      if (current_stasis != stasis[n]):
        current_stasis = stasis[n]
        cmd = cmd_psalm_division
        cmd = cmd.replace("$1", str(current_kathisma))
        cmd = cmd.replace("$2", str(current_stasis))
    ...
    if insc:
      cmd = cmd_psalm_inscription
      cmd = cmd.replace("$1", str(n))
      cmd = cmd.replace("$2", insc)
      #f.write(cmd_psalm_inscription.replace("$1", insc))
      f.write(cmd)
    else:
      f.write(cmd_psalm_chapter.replace("$1", str(n)))

    for verse in verses:
      # Not sure if this is the right place. Want consistent spacing for verse
      # numbers in the text.
      phantom = ""
      if (10 > int(verse)):
        phantom = r"\phantom{00}"
      elif (100 > int(verse)):
        phantom = r"\phantom{0}"

      cmd = cmd_psalm_verse
      cmd = cmd.replace("$1", phantom + verse)
      cmd = cmd.replace("$2", la[verse])
      f.write(cmd)
